...
<c:forEach items="${messages}" var="message" varStatus="index">
    <tr>
	    ...
	    <td>${message.message}
	      <c:forEach items="${message.tags}" var="tag" >
	        <span class="label label-info">
	        	<a href="/search/${tag.id}" style="color:white;">${tag.name}</a>
	        </span>
	        &nbsp;          
	      </c:forEach>
	    </td>
		  ...
		  <div class="form-group">
		    <label class="control-label" for="tag">Tag:</label>		    
		   	   <c:forEach items="${tags}" var="tag">
		      	 <input type="checkbox" id="chkTags" name="chkTags" value="${tag.id}" />${tag.name} &nbsp;
		      </c:forEach>		    
		  </div>
